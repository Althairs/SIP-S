<div>
    ...
    <nav class="flex items-center text-sm text-gray-500 mb-6">
        ...
        @else
            <span class="text-green-700 font-medium">Jadwal Menguji</span>
        @endif
    </nav>
            ...
            <div>
                ...
                <div class="flex gap-4 mt-3">
                    <div class="bg-white/20 rounded-lg px-3 py-1.5 text-sm">
                        <span class="text-green-200">Akan Datang:</span>
                        <span class="font-bold ml-1">{{ $jadwalAkanDatang->count() }}</span>
                    </div>
                    <div class="bg-white/20 rounded-lg px-3 py-1.5 text-sm">
                        <span class="text-green-200">Selesai:</span>
    ...
    <div class="flex border-b border-gray-200 mb-6">
        <button wire:click="setTab('upcoming')" class="px-6 py-3 text-sm font-medium border-b-2 transition {{ $tab === 'upcoming' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
            ...
            <span class="ml-2 px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs">{{ $jadwalAkanDatang->count() }}</span>
            @endif
        </button>
        <button wire:click="setTab('history')" class="px-6 py-3 text-sm font-medium border-b-2 transition {{ $tab === 'history' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
    ...
    @if($tab === 'upcoming')
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
            <div class="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg class="w-10 h-10 text-green-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            </div>
        ...
        <div class="space-y-4">
            ...
            <div class="bg-white rounded-2xl shadow-sm border border-green-100 hover:shadow-md transition cursor-pointer" wire:click="showDetailUjian({{ $p->id }})">
                <div class="p-6">
                        ...
                        <div class="flex items-start space-x-4">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2 flex-wrap">
                                    <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">{{ ucwords(str_replace('_', ' ', $p->jenis_ujian)) }}</span>
                ...
                <tbody class="divide-y">
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-center">
                            <button wire:click="showDetailUjian({{ $p->id }})" class="text-sm text-green-600 hover:underline">Lihat</button>
                        </td>
                    </tr>
                    @endforeach
                </tbody>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-green-200 overflow-hidden">
        ...
        <div class="bg-gradient-to-r from-green-600 to-green-800 px-6 py-4 flex justify-between items-center">
            <div>
                <span class="px-3 py-1 bg-white/20 rounded-full text-xs text-white">{{ ucwords(str_replace('_', ' ', $p->jenis_ujian)) }}</span>
                <h2 class="text-xl font-bold mt-2 text-white">Detail Ujian</h2>
            </div>
            <button wire:click="closeDetail" class="text-white hover:text-gray-200 bg-white/10 rounded-full p-2">
        ...
        <div class="p-6 space-y-6">
            ...
            <!-- Mahasiswa -->
            <div class="bg-green-50 rounded-xl p-4 flex items-center space-x-3">
            ...
            <div>
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    @if($p->pengujis->count() > 0)
                        ...
                        <div class="border rounded-xl p-4 {{ $penguji->dosen_id == auth()->id() ? 'border-green-400 bg-green-50' : 'border-gray-200' }}">
                            <div class="flex items-center space-x-3">
                                ...
                                <div>
                                    ...
                                    @if($penguji->dosen->kepakaran)<p class="text-xs text-green-600">{{ $penguji->dosen->kepakaran->nama_kepakaran }}</p>@endif
                                </div>
                            </div>
                        </div>
                        @endforeach
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-2">
                    ...
                    <span class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm">{{ $bk->nama_bidang }}</span>
                    @endforeach
                </div>
            </div>
            @endif
            ...
            <div>
                ...
                <div class="flex flex-wrap gap-3">
                    @if($p->file_proposal)
                    <a href="{{ asset('storage/' . $p->file_proposal) }}" target="_blank" class="px-4 py-2 bg-green-50 text-green-700 rounded-xl text-sm hover:bg-green-100 flex items-center">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        Proposal
                    </a>
                    @endif
                    @if($p->file_skripsi)
                    <a href="{{ asset('storage/' . $p->file_skripsi) }}" target="_blank" class="px-4 py-2 bg-green-50 text-green-700 rounded-xl text-sm hover:bg-green-100 flex items-center">
                        <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"></path></svg>
                        Skripsi
                    </a>
                    @endif
