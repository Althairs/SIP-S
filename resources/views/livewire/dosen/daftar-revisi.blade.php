<div>
    ...
    <div x-data="{ tab: 'pending' }">
        ...
        <div class="flex border-b border-gray-200 mb-6 space-x-2 overflow-x-auto">
            <button @click="tab = 'pending'" :class="tab === 'pending' ? 'border-amber-600 text-amber-700 bg-amber-50 rounded-t-lg' : 'border-transparent text-gray-500 hover:bg-gray-50'" class="px-6 py-3 text-sm font-medium border-b-2 transition whitespace-nowrap">Perlu Direvisi (Sebagai Penguji)</button>
            <button @click="tab = 'history'" :class="tab === 'history' ? 'border-amber-600 text-amber-700 bg-amber-50 rounded-t-lg' : 'border-transparent text-gray-500 hover:bg-gray-50'" class="px-6 py-3 text-sm font-medium border-b-2 transition whitespace-nowrap">Riwayat Revisi (Sebagai Penguji)</button>
            <button @click="tab = 'bimbingan'" :class="tab === 'bimbingan' ? 'border-amber-600 text-amber-700 bg-amber-50 rounded-t-lg' : 'border-transparent text-gray-500 hover:bg-gray-50'" class="px-6 py-3 text-sm font-medium border-b-2 transition whitespace-nowrap">Revisi Bimbingan</button>
        </div>
    ...
    <div class="space-y-3">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-green-200 p-4 bg-green-50/20">
            ...
            <div class="mt-2 flex justify-between items-center text-xs text-gray-500 border-t pt-2">
                ...
                <a href="{{ asset('storage/' . $rev->file_revisi_mahasiswa) }}" target="_blank" class="text-blue-600 hover:underline">Lihat Arsip File</a>
                @endif
            </div>
        </div>
        @endforeach
                                ...
                                @else
                                    ...
                                    <div class="bg-gray-50 rounded-xl p-4 border border-gray-100 relative">
                                        <div class="flex justify-between">
                                            ...
                                            <span class="px-2 py-0.5 rounded-full text-xs font-medium
                                                {{ $revisi->status == 'pending' ? 'bg-gray-200 text-gray-700' :
                                                  ($revisi->status == 'diperiksa' ? 'bg-blue-200 text-blue-700' : 'bg-green-200 text-green-700') }}">
                                        ...
                                        <div class="mt-2 border-t pt-2">
                                            <a href="{{ asset('storage/' . $revisi->file_revisi_mahasiswa) }}" target="_blank" class="inline-flex items-center text-xs text-blue-600 hover:text-blue-800">
    ...
    @if($selectedRevisiId)
        ...
        <div class="fixed inset-0 z-50 overflow-y-auto" aria-labelledby="modal-title" role="dialog" aria-modal="true">
            <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20 text-center sm:block sm:p-0">
                <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" wire:click="$set('selectedRevisiId', null)"></div>
                ...
                <div class="inline-block align-bottom bg-white rounded-2xl text-left overflow-hidden shadow-xl transform transition-all sm:my-8 sm:align-middle sm:max-w-3xl sm:w-full">
                    <div class="bg-gradient-to-r from-green-600 to-green-800 px-6 py-4 flex justify-between items-center">
                        <h3 class="text-lg leading-6 font-semibold text-white" id="modal-title">
                            Review & Persetujuan Revisi
                        </h3>
                        <button wire:click="$set('selectedRevisiId', null)" class="text-white hover:text-gray-200">
                    ...
                    <div class="px-6 py-5">
                        ...
                        <div class="mb-5 bg-blue-50 p-4 rounded-xl border border-blue-100">
                            <p class="text-xs text-blue-600 font-medium mb-2">File Revisi Mahasiswa</p>
                            <a href="{{ asset('storage/' . $selectedRevisi->file_revisi_mahasiswa) }}" target="_blank" class="inline-flex items-center px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-sm">
