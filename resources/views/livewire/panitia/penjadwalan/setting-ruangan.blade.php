        ...
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div class="relative flex-1">
                <input type="text" wire:model.live.debounce.300ms="search" placeholder="Cari ruangan..."
                       class="w-full md:w-96 pl-10 pr-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl shadow-sm border {{ $ruangan->is_active ? 'border-gray-100' : 'border-red-200 opacity-60' }} p-6">
            ...
            <div class="mt-3 flex items-center gap-2">
                <button wire:click="openEditModal({{ $ruangan->id }})" class="flex-1 px-3 py-1.5 bg-amber-50 text-amber-700 rounded-lg hover:bg-amber-100 text-sm font-medium text-center">Edit</button>
        ...
        <div class="flex items-center justify-center min-h-screen px-4">
            ...
            <div class="relative bg-white rounded-2xl shadow-xl max-w-md w-full p-6">
                <div class="flex items-center justify-between mb-4">
                    <h3 class="text-lg font-bold">{{ $editMode ? 'Edit' : 'Tambah' }} Ruangan</h3>
                    ...
                    <div class="space-y-4">
                        <div class="grid grid-cols-2 gap-4">
                            <div>
                                ...
                                <input type="text" wire:model="kode_ruangan" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 @error('kode_ruangan') border-red-500 @enderror" placeholder="R001">
                            ...
                            <div>
                                ...
                                <input type="number" wire:model="kapasitas" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500" min="1" max="100">
                            </div>
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Nama Ruangan <span class="text-red-500">*</span></label>
                            <input type="text" wire:model="nama_ruangan" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500 @error('nama_ruangan') border-red-500 @enderror">
                            @error('nama_ruangan') <p class="mt-1 text-xs text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Lokasi</label>
                            <input type="text" wire:model="lokasi" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500" placeholder="Lt. 2, Gedung A">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Deskripsi</label>
                            <textarea wire:model="deskripsi" rows="2" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-teal-500"></textarea>
                        </div>
