<div>
    ...
    <nav class="flex items-center text-sm text-gray-500 mb-4">
        <a href="{{ route('panitia.penjadwalan.jadwal') }}" class="hover:text-green-700 transition">Penjadwalan</a>
        <span class="mx-2">/</span>
        <a href="{{ route('panitia.penjadwalan.jadwal') }}" class="hover:text-green-700 transition">Jadwal Ujian</a>
        @if($modalMode === 'schedule')
            <span class="mx-2">/</span>
            <span class="text-green-700 font-medium">Jadwalkan</span>
        @elseif($modalMode === 'batch')
            <span class="mx-2">/</span>
            <span class="text-green-700 font-medium">Batch</span>
        @elseif($modalMode === 'detail')
            <span class="mx-2">/</span>
    ...
    {{-- ============= SCHEDULE CARD ============= --}}
    @if($modalMode === 'schedule' && $selectedPendaftaran)
        <div class="bg-white rounded-2xl shadow-sm border border-green-200 p-6">
            ...
            <div class="mb-4 p-4 bg-gray-50 rounded-xl space-y-2">
                <span
                    class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs">{{ ucwords(str_replace('_', ' ', $selectedPendaftaran->jenis_ujian)) }}</span>
                ...
                <div class="grid grid-cols-2 gap-2 text-xs">
                    ...
                    <div><span class="text-gray-500">Min. Ujian:</span> <span
                            class="font-medium text-green-600">{{ $tanggal_minimal ? $tanggal_minimal->format('d M Y') : '-' }}</span>
                    </div>
                </div>
            ...
            <div class="mb-4 p-3 bg-amber-50 border border-amber-200 rounded-xl text-sm">
                ...
                <p class="text-xs text-amber-700 mt-1">Minimal <strong>7 hari</strong> setelah pendaftaran →
                    ...
                    mengikuti sesi yang dipilih<br>• Sesi diatur di <a
                        ...
                        class="text-green-600 underline">Pengaturan Waktu</a></p>
            </div>

            <div class="flex gap-3 mb-4">
                <button type="button" wire:click="autoGenerateJadwal"
                    class="px-4 py-2 {{ $scheduleMode === 'auto' ? 'bg-green-700 text-white' : 'bg-gray-100 text-gray-700' }} rounded-xl text-sm font-medium transition flex items-center">
                    Random</button>
            </div>
            ...
            <form wire:submit="scheduleUjian">
                <div class="space-y-4">
                    <div>
                        ...
                        <input type="date" wire:model="tanggal_ujian"
                            ...
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('tanggal_ujian') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-3">
                            @foreach($labelSesiOptions as $index => $label)
                                <label
                                    class="flex items-center p-3 border-2 rounded-xl cursor-pointer transition {{ ($sesi == ($index + 1)) ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300' }}"
                                    wire:click="$set('sesi', {{ $index + 1 }})">
                                    <input type="radio" wire:model="sesi" value="{{ $index + 1 }}" class="hidden">
                                    <div class="flex-1">
                                        <p
                                            class="text-sm font-medium {{ ($sesi == ($index + 1)) ? 'text-green-700' : 'text-gray-900' }}">
                                            {{ $label }}</p>
                                        <p
                                            class="text-xs {{ ($sesi == ($index + 1)) ? 'text-green-600' : 'text-gray-500' }}">
                        ...
                        <p class="text-xs text-gray-400 mt-1"><a
                                href="{{ route('panitia.penjadwalan.setting-waktu') }}"
                                class="text-green-600 hover:underline">+ Kelola Waktu & Sesi</a></p>
                    ...
                    <div>
                        ...
                        <select wire:model="ruangan"
                            class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('ruangan') border-red-500 @enderror">
                            <option value="">Pilih Ruangan</option>
                            @foreach($ruanganOptions as $r)<option value="{{ $r }}">{{ $r }}</option>@endforeach
                        </select>
                        <p class="text-xs text-gray-400 mt-1"><a
                                href="{{ route('panitia.penjadwalan.setting-ruangan') }}"
                                class="text-green-600 hover:underline">+ Kelola Ruangan</a></p>
                    ...
                    @if($tanggal_ujian && $sesi)
                        ...
                        <div class="p-4 bg-green-50 rounded-xl border border-green-200">
                            ...
                            <div class="grid grid-cols-3 gap-3 mt-2 text-sm">
                                <div>
                                    <p class="text-xs text-green-600">Tanggal</p>
                                    <p class="font-bold text-green-800">
                                        {{ \Carbon\Carbon::parse($tanggal_ujian)->format('d M Y') }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-green-600">Sesi</p>
                                    <p class="font-bold text-green-800">{{ $labelSesiOptions[$si] ?? 'Sesi ' . $sesi }}</p>
                                </div>
                                <div>
                                    <p class="text-xs text-green-600">Waktu</p>
                                    <p class="font-bold text-green-800">{{ $jamMulaiOptions[$si] ?? '-' }} -
                ...
                <div class="mt-6 flex justify-end gap-4">
                    ...
                    <button type="submit"
                        class="px-6 py-2.5 bg-green-700 text-white rounded-xl hover:bg-green-800 font-medium shadow-sm shadow-green-200">
            ...
            <form wire:submit="scheduleBatchUjian">
                ...
                <div class="mt-6 flex justify-end gap-4">
                    <button type="button" wire:click="closeBatchModal"
                        class="px-6 py-2.5 border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 font-medium">Batal</button>
                    <button type="submit"
                        class="px-6 py-2.5 bg-green-700 text-white rounded-xl hover:bg-green-800 font-medium">Jadwalkan
    ...
    {{-- ============= DETAIL CARD ============= --}}
    @elseif($modalMode === 'detail' && $selectedPendaftaran)
        <div class="bg-white rounded-2xl shadow-sm border border-green-200 p-8">
            ...
            <div class="space-y-6">
                    ...
                    <div class="bg-gray-50 rounded-xl p-6">
                        ...
                        <div class="flex flex-wrap gap-2">
                            @foreach($selectedPendaftaran->bidangKeahlians as $bk)
                                <span class="px-3 py-1 bg-green-100 text-green-800 rounded-full text-sm font-medium">{{ $bk->nama_bidang }}</span>
                            @endforeach
                        </div>
                    </div>
                @endif
                ...
                @if($selectedPendaftaran->tanggal_ujian)
                    <div class="bg-green-50 rounded-xl p-6">
                        <p class="text-sm text-green-600 mb-2">Jadwal Ujian</p>
                        <p class="text-lg font-semibold text-green-900">{{ \Carbon\Carbon::parse($selectedPendaftaran->tanggal_ujian)->format('d M Y H:i') }}</p>
                        <p class="text-base text-green-800">{{ $selectedPendaftaran->ruangan ?? '-' }} | Sesi {{ $selectedPendaftaran->sesi ?? '-' }}</p>
                    </div>
                @endif
            </div>
    ...
    @else
        ...
        <div class="flex border-b border-gray-200 mb-6 gap-1 overflow-x-auto">
            <button wire:click="setTab('siap')"
                class="px-6 py-3 text-sm font-medium border-b-2 transition whitespace-nowrap {{ $tab === 'siap' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                <svg class="w-5 h-5 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
            ...
            <button wire:click="setTab('scheduled')"
                class="px-6 py-3 text-sm font-medium border-b-2 transition whitespace-nowrap {{ $tab === 'scheduled' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                <svg class="w-5 h-5 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                        d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                </svg>
                ...
                class="ml-2 px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs">{{ $countScheduled }}</span>@endif
            </button>
            <button wire:click="setTab('completed')"
                class="px-6 py-3 text-sm font-medium border-b-2 transition whitespace-nowrap {{ $tab === 'completed' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                <svg class="w-5 h-5 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    <div class="flex items-center justify-between">
                        <div class="flex items-center gap-3">
                            ...
                            <span class="text-sm text-gray-600">
                                ...
                                @if(count($selectedIds) > 0)<span class="ml-2 font-medium text-green-700">{{ count($selectedIds) }}
                                terpilih</span>@endif
                            </span>
                        </div>
                        <button wire:click="openBatchModal"
                            class="px-5 py-2 bg-green-700 text-white rounded-xl hover:bg-green-800 transition font-medium text-sm {{ count($selectedIds) == 0 ? 'opacity-50 cursor-not-allowed' : '' }}"
                        ...
                        <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                            <div class="flex items-start space-x-3">
                                ...
                                <div class="flex-1">
                                    <div class="flex items-center gap-2 mb-2 flex-wrap">
                                        <span
                                            class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">{{ ucwords(str_replace('_', ' ', $p->jenis_ujian)) }}</span>
                                        <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">Disetujui
                                            Kajur</span>
                                        <span
                                    ...
                                    @if($p->bidangKeahlians->count() > 0)
                                        <div class="flex flex-wrap gap-1 mt-2">
                                            @foreach($p->bidangKeahlians as $bk)
                                                <span
                                                    class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs font-medium">{{ $bk->nama_bidang }}</span>
                                            @endforeach
                                        </div>
                                    @endif
                                </div>
                            ...
                            <div class="md:text-right md:flex-shrink-0">
                                ...
                                <button wire:click="openScheduleModal({{ $p->id }})"
                                    class="px-5 py-2.5 bg-green-700 text-white rounded-xl hover:bg-green-800 text-sm font-medium whitespace-nowrap shadow-sm shadow-green-200 transition">
                                    <svg class="w-4 h-4 inline mr-1" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                            d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z" />
                                    </svg>
                        ...
                        <div class="flex flex-col md:flex-row md:items-start justify-between gap-4">
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2">
                                    <span
                                        class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">{{ ucwords(str_replace('_', ' ', $p->jenis_ujian)) }}</span>
                                    <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs">Terjadwal</span>
                                </div>
                                <h3 class="font-semibold text-gray-900">{{ Str::limit($p->judul_penelitian, 60) }}</h3>
                                <div class="mt-3 bg-green-50 rounded-xl p-4">
                                    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                                        ...
                                        <div>
                                            <p class="text-xs text-green-600">Sesi</p>
                                            <p class="font-bold text-green-800 text-sm">{{ $ls }}</p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-green-600">Waktu</p>
                                            <p class="font-bold text-green-800 text-sm">{{ $jm }} - {{ $js }} WIB</p>
                                        </div>
                                        <div>
                                            <p class="text-xs text-green-600">Ruangan</p>
                                            <p class="font-bold text-green-800 text-sm">{{ $p->ruangan ?? '-' }}</p>
                                        </div>
                                    </div>
                                </div>
            ...
            <div class="space-y-3">
                @forelse($pendaftarans as $p)
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 opacity-80">
                        ...
                        @if($p->nilai_total)
                            <p class="text-sm mt-1">Nilai: <span class="font-bold text-green-700">{{ $p->nilai_total }}
                        ({{ $p->grade }})</span></p>@endif
                        <div class="mt-2">
                            <button wire:click="openDetailModal({{ $p->id }})"
                                class="text-xs text-green-600 hover:text-green-800 font-medium">Lihat Detail</button>
                        </div>
                    </div>
                @empty
                    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center text-gray-500">Belum ada
