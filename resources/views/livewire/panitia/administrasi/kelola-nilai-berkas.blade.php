<div>
                    ...
                    <tr class="hover:bg-gray-50">
                        ...
                        <td class="px-6 py-4 text-center">
                            <div class="flex items-center justify-center gap-2">
                                <!-- Tombol Input Nilai -->
                                <button wire:click="openInputModal({{ $penilaian->id }})" class="px-3 py-1.5 bg-indigo-50 text-indigo-700 rounded-lg hover:bg-indigo-100 text-xs font-medium inline-flex items-center gap-1" title="Input Nilai ke Sistem">
                                ...
                                <a href="{{ asset('storage/' . $penilaian->file_penilaian) }}" target="_blank" class="px-3 py-1.5 bg-blue-50 text-blue-700 rounded-lg hover:bg-blue-100 text-xs font-medium inline-flex items-center gap-1" title="Download Berkas">
                                    <svg class="w-3.5 h-3.5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 16v1a3 3 0 003 3h10a3 3 0 003-3v-1m-4-4l-4 4m0 0l-4-4m4 4V4"/></svg>
                                    Berkas
                                </a>
                                @endif

                                <button wire:click="selectPenilaian({{ $penilaian->id }})" class="px-3 py-1.5 bg-gray-50 text-gray-700 rounded-lg hover:bg-gray-100 text-xs font-medium inline-flex items-center gap-1">
    ...
    <!-- MODAL INPUT NILAI OLEH PANITIA -->
    @if($inputPenilaianId && $penilaianToInput)
    <div class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75 transition-opacity" wire:click="closeInputModal"></div>
            <div class="relative bg-white rounded-2xl shadow-xl max-w-4xl w-full overflow-hidden">
                <div class="bg-gradient-to-r from-indigo-700 to-purple-800 px-6 py-4 flex justify-between items-center text-white">
                    <div>
                        ...
                        <p class="text-xs text-indigo-200">Mahasiswa: {{ $penilaianToInput->pendaftaran->mahasiswa->name }} | Dosen: {{ $penilaianToInput->dosen->name }}</p>
                    </div>
                    <button wire:click="closeInputModal" class="text-white hover:text-gray-200">
                ...
                <div class="p-6 max-h-[80vh] overflow-y-auto space-y-6">
                    ...
                    <div class="bg-blue-50 border border-blue-200 rounded-xl p-4 flex items-center justify-between">
                        <div class="flex items-center space-x-3">
                            <svg class="w-8 h-8 text-blue-600 flex-shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z"/></svg>
                            <div>
                                <p class="text-sm font-bold text-blue-900">Dokumen Penilaian Dosen</p>
                                <p class="text-xs text-blue-700">Silakan buka atau unduh dokumen untuk melihat rincian nilai yang diberikan dosen.</p>
                            </div>
                        </div>
                        @if($penilaianToInput->file_penilaian)
                        <a href="{{ asset('storage/' . $penilaianToInput->file_penilaian) }}" target="_blank" class="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 text-xs font-semibold shadow flex items-center gap-1.5">
                    ...
                    <form wire:submit="simpanNilai">
                            ...
                            <div class="flex items-center gap-4 p-3 bg-gray-50 rounded-xl border border-gray-100">
                                <div class="flex-1">
                                    ...
                                    <div class="flex items-center gap-3 mt-1">
                                        ...
                                        <input type="number" wire:model.live="{{ $key }}" min="0" max="100" step="0.01" class="w-24 px-3 py-1.5 border border-gray-300 rounded-lg text-center text-sm font-bold focus:ring-2 focus:ring-indigo-500">
                        ...
                        <div class="mt-6 bg-gradient-to-r from-indigo-50 to-purple-50 rounded-2xl p-5 border border-indigo-100">
                            <h4 class="text-sm font-bold text-indigo-900 mb-3">Hasil Kalkulasi Otomatis:</h4>
                            <div class="grid grid-cols-3 gap-4 text-center">
                                <div class="bg-white p-3 rounded-xl shadow-sm">
                                    <p class="text-xs text-gray-500 font-medium">Nilai Akhir (NA)</p>
                                    <p class="text-2xl font-black text-indigo-700">{{ $nilaiAkhir }}</p>
                                </div>
                                <div class="bg-white p-3 rounded-xl shadow-sm">
                                    <p class="text-xs text-gray-500 font-medium">Nilai Huruf</p>
                                    <p class="text-2xl font-black {{ $nilaiHuruf === 'A' ? 'text-green-600' : ($nilaiHuruf === 'B' ? 'text-blue-600' : 'text-amber-600') }}">{{ $nilaiHuruf }}</p>
                                </div>
                                <div class="bg-white p-3 rounded-xl shadow-sm">
                        ...
                        <div class="mt-4">
                            ...
                            <textarea wire:model="catatanPanitia" rows="2" class="w-full px-4 py-2 border border-gray-300 rounded-xl text-sm focus:ring-2 focus:ring-indigo-500" placeholder="Tambahkan catatan jika ada..."></textarea>
                        </div>

                        <div class="mt-6 flex justify-end gap-3 pt-4 border-t">
                            <button type="button" wire:click="closeInputModal" class="px-5 py-2.5 border border-gray-300 text-gray-700 rounded-xl hover:bg-gray-50 font-medium text-sm">Batal</button>
                            <button type="submit" class="px-6 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 font-medium text-sm shadow-sm inline-flex items-center gap-2">
    ...
    <!-- Modal Detail (Bawaan Sebelumnya) -->
    @if($selectedPenilaian)
    <div class="fixed inset-0 z-50 overflow-y-auto">
        <div class="flex items-center justify-center min-h-screen px-4 pt-4 pb-20">
            <div class="fixed inset-0 bg-gray-500 bg-opacity-75" wire:click="closeDetail"></div>
            <div class="relative bg-white rounded-2xl shadow-xl max-w-2xl w-full overflow-hidden">
                ...
                <div class="px-6 py-5 space-y-4">
                    ...
                    <div class="flex gap-3">
                        <a href="{{ asset('storage/' . $selectedPenilaian->file_penilaian) }}" target="_blank" class="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 bg-blue-600 text-white rounded-xl hover:bg-blue-700 text-sm font-medium">
