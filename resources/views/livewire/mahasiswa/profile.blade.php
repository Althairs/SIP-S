    ...
    <div class="max-w-4xl mx-auto">
        ...
        <div class="bg-gradient-to-r from-green-600 to-green-800 rounded-2xl p-6 mb-6 text-white">
            <div class="flex items-center space-x-4">
                <img class="w-20 h-20 rounded-full border-4 border-white/30"
                     src="{{ $currentFoto ? asset('storage/' . $currentFoto) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&size=80&background=16a34a&color=fff' }}"
                     alt="{{ Auth::user()->name }}">
                <div>
                    <h2 class="text-2xl font-bold">{{ Auth::user()->name }}</h2>
                    <p class="text-green-100">{{ Auth::user()->nim }} | {{ Auth::user()->jurusan?->nama_jurusan }}</p>
                    <p class="text-green-200 text-sm">{{ Auth::user()->prodi?->nama_prodi }}</p>
                </div>
            </div>
        </div>
        ...
        <div class="flex border-b border-gray-200 mb-6">
            <button wire:click="setTab('profile')"
                    class="px-6 py-3 text-sm font-medium border-b-2 transition {{ $activeTab === 'profile' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                Edit Profile
            </button>
            <button wire:click="setTab('password')"
                    class="px-6 py-3 text-sm font-medium border-b-2 transition {{ $activeTab === 'password' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                Ubah Password
            </button>
            <button wire:click="setTab('info')"
                    class="px-6 py-3 text-sm font-medium border-b-2 transition {{ $activeTab === 'info' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700' }}">
                Informasi Akademik
            </button>
        </div>
                ...
                <div class="space-y-6">
                    ...
                    <div>
                        ...
                        <div class="flex items-center space-x-4">
                            <img class="w-16 h-16 rounded-full"
                                 src="{{ $currentFoto ? asset('storage/' . $currentFoto) : 'https://ui-avatars.com/api/?name=' . urlencode(Auth::user()->name) . '&size=64&background=16a34a&color=fff' }}"
                                 alt="">
                            <div>
                    ...
                    <div class="grid md:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="text" wire:model="name" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('name') border-red-500 @enderror">
                            @error('name') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Email <span class="text-red-500">*</span></label>
                            <input type="email" wire:model="email" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('email') border-red-500 @enderror">
                        ...
                        <div>
                            ...
                            <input type="text" wire:model="nomor_hp" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500" placeholder="0812xxxxxx">
                        </div>
                        <div>
                            <label class="block text-sm font-medium text-gray-700 mb-1">Angkatan</label>
                            <input type="text" wire:model="angkatan" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500" placeholder="2021">
                        </div>
                        <div class="md:col-span-2">
                            <label class="block text-sm font-medium text-gray-700 mb-1">Alamat</label>
                            <textarea wire:model="alamat" rows="3" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500" placeholder="Alamat lengkap..."></textarea>
                        </div>
                    </div>
                </div>

                <div class="mt-6 flex justify-end">
                    <button type="submit" class="px-6 py-2.5 bg-green-700 text-white rounded-xl hover:bg-green-800 transition font-medium">
                        Simpan Profile
                    </button>
                </div>
            </form>
            ...
            <form wire:submit="updatePassword">
                <div class="space-y-4 max-w-md">
                    <div>
                        ...
                        <input type="password" wire:model="current_password" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('current_password') border-red-500 @enderror">
                        @error('current_password') <p class="mt-1 text-sm text-red-600">{{ $message }}</p> @enderror
                    </div>
                    <div>
                        <label class="block text-sm font-medium text-gray-700 mb-1">Password Baru <span class="text-red-500">*</span></label>
                        <input type="password" wire:model="password" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 @error('password') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="password" wire:model="password_confirmation" class="w-full px-4 py-2.5 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500">
                    </div>
                </div>
