<div>
    ...
    <nav class="mb-6 flex items-center text-sm text-gray-500">
        ...
        @if($showDetail && $selectedUjian)
            <a href="#" wire:click="closeDetail" class="text-green-600 hover:text-green-700 font-medium">Jadwal Ujian</a>
            <svg class="w-4 h-4 mx-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
            <span class="text-gray-700 font-medium">Detail — {{ $selectedUjian->jenis_ujian }}</span>
        @else
            <span class="text-gray-700 font-medium">Jadwal Ujian</span>
        @endif
    </nav>

    @unless($showDetail && $selectedUjian)

    <!-- Tab Navigation & Content -->
    <div x-data="{ tab: 'upcoming' }">
        <div class="flex border-b border-gray-200 mb-6">
            <button @click="tab = 'upcoming'"
                    :class="tab === 'upcoming' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700'"
                ...
                <span class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs">{{ $upcomingUjian->count() }}</span>
                @endif
            </button>
            <button @click="tab = 'history'"
                    :class="tab === 'history' ? 'border-green-600 text-green-700' : 'border-transparent text-gray-500 hover:text-gray-700'"
                    class="px-6 py-3 text-sm font-medium border-b-2 transition flex items-center gap-2">
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-12 text-center">
            <div class="w-20 h-20 bg-green-100 rounded-full flex items-center justify-center mx-auto mb-4">
                <svg class="w-10 h-10 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M8 7V3m8 4V3m-9 8h10M5 21h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v12a2 2 0 002 2z"/></svg>
            </div>
            ...
            <a href="{{ route('mahasiswa.pendaftaran.create') }}" class="inline-flex px-5 py-2.5 bg-green-700 text-white rounded-xl hover:bg-green-800 font-medium">Daftar Ujian</a>
        </div>
        @else
        <div class="space-y-4">
            @foreach($upcomingUjian as $ujian)
            <div class="bg-white rounded-2xl shadow-sm border border-green-100 hover:shadow-md transition cursor-pointer" wire:click="showDetailUjian({{ $ujian->id }})">
                <div class="p-6">
                    <div class="flex items-start justify-between">
                        <div class="flex items-start space-x-4">
                            <!-- Tanggal Box -->
                            <div class="bg-gradient-to-br from-green-600 to-green-700 text-white rounded-xl p-3 text-center min-w-[75px] shadow-lg shadow-green-200">
                            ...
                            <div class="flex-1">
                                <div class="flex items-center gap-2 mb-2">
                                    ...
                                    <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">Terjadwal</span>
                                </div>
                                ...
                                <div class="mt-3 grid grid-cols-1 md:grid-cols-3 gap-3">
                                    <div class="flex items-center text-sm text-gray-600">
                                        ...
                                        <span class="ml-1 text-green-600">(Sesi {{ $ujian->sesi }})</span>
                                        @endif
                                    </div>
                                    <div class="flex items-center text-sm text-gray-600">
                ...
                <table class="w-full">
                    ...
                    <tbody class="divide-y divide-gray-200">
                        ...
                        <tr class="hover:bg-gray-50">
                            ...
                            <td class="px-6 py-4">
                                <button wire:click="showDetailUjian({{ $ujian->id }})" class="text-sm text-green-700 hover:text-green-800 font-medium">Lihat Detail</button>
                            </td>
                        </tr>
                        @endforeach
                    </tbody>
            ...
            <div class="bg-gradient-to-r from-green-600 to-green-800 p-6 text-white">
                ...
                <div class="p-6 space-y-6">
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
                                <p class="text-xs text-green-600 font-medium mb-1">Pembimbing 1</p>
                                @if($p1 && $p1->dosen)
                                <div class="flex items-center space-x-3">
                                    <img class="w-12 h-12 rounded-full bg-green-200" src="https://ui-avatars.com/api/?name={{ urlencode($p1->dosen->name) }}&background=16a34a&color=fff" alt="">
                                    <div>
                                        <p class="font-bold text-gray-900">{{ $p1->dosen->name }}</p>
                                        <p class="text-xs text-gray-500">NIP: {{ $p1->dosen->nip ?? '-' }}</p>
                                        @if($p1->dosen->kepakaran)
                                        <span class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs">{{ $p1->dosen->kepakaran->nama_kepakaran }}</span>
                                        @endif
                                    </div>
                                </div>
                                @else
                            ...
                            <div class="bg-gradient-to-br from-green-50 to-green-100 rounded-xl p-4 border border-green-200">
                                <p class="text-xs text-green-600 font-medium mb-1">Pembimbing 2</p>
                                @if($p2 && $p2->dosen)
                                <div class="flex items-center space-x-3">
                                    <img class="w-12 h-12 rounded-full bg-green-200" src="https://ui-avatars.com/api/?name={{ urlencode($p2->dosen->name) }}&background=16a34a&color=fff" alt="">
                                    <div>
                                        <p class="font-bold text-gray-900">{{ $p2->dosen->name }}</p>
                                        <p class="text-xs text-gray-500">NIP: {{ $p2->dosen->nip ?? '-' }}</p>
                                        @if($p2->dosen->kepakaran)
                                        <span class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs">{{ $p2->dosen->kepakaran->nama_kepakaran }}</span>
                                        @endif
                                    </div>
                                </div>
                                @else
                            ...
                            <div class="bg-gradient-to-br from-amber-50 to-amber-100 rounded-xl p-4 border border-amber-200">
                                ...
                                <div class="flex items-center space-x-3">
                                    ...
                                    <div>
                                        ...
                                        <span class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs">{{ $penguji->dosen->kepakaran->nama_kepakaran }}</span>
                                        @endif
                                    </div>
                                </div>
                    ...
                    <div class="bg-gradient-to-r from-green-50 to-green-100 rounded-2xl p-6 border border-green-200">
                        <h3 class="text-lg font-bold text-green-800 mb-4">Hasil Ujian</h3>
                        <div class="grid grid-cols-3 gap-6 text-center">
                            <div>
                                <p class="text-sm text-green-600 mb-1">Nilai Total</p>
                                <p class="text-4xl font-bold text-green-800">{{ $selectedUjian->nilai_total }}</p>
                            </div>
                            <div>
                                <p class="text-sm text-green-600 mb-1">Grade</p>
                                <p class="text-4xl font-bold text-green-800">{{ $selectedUjian->grade }}</p>
                            </div>
                            <div>
                        ...
                        <div class="mt-4 bg-white/50 rounded-xl p-3">
                            <p class="text-xs text-green-600 font-medium mb-1">Catatan Penguji</p>
                    ...
                    <div>
                        ...
                        <div class="grid grid-cols-2 gap-3">
                            ...
                            <a href="{{ asset('storage/' . $selectedUjian->file_proposal) }}" target="_blank" class="flex items-center gap-2 p-3 bg-gray-50 rounded-xl hover:bg-green-50 transition border border-gray-200">
                            ...
                            <a href="{{ asset('storage/' . $selectedUjian->file_skripsi) }}" target="_blank" class="flex items-center gap-2 p-3 bg-gray-50 rounded-xl hover:bg-green-50 transition border border-gray-200">
                                <svg class="w-5 h-5 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M7 21h10a2 2 0 002-2V9.414a1 1 0 00-.293-.707l-5.414-5.414A1 1 0 0012.586 3H7a2 2 0 00-2 2v14a2 2 0 002 2z"/></svg>
