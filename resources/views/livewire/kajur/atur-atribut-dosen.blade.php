<div>
    ...
    <nav class="mb-4 text-sm text-gray-500">
        <ol class="flex items-center space-x-1">
            <li><a href="{{ route('kajur.data-master.atur-atribut-dosen') }}" class="hover:text-green-700">Data Master</a></li>
            <li><span class="mx-1">/</span></li>
            @if($showForm)
            <li><a href="{{ route('kajur.data-master.atur-atribut-dosen') }}" class="hover:text-green-700">Atribut Dosen</a></li>
            <li><span class="mx-1">/</span></li>
            <li class="text-gray-900 font-medium">Edit</li>
            @else
            <li class="text-gray-900 font-medium">Atribut Dosen</li>
            @endif
        </ol>
    </nav>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-6">
        <div class="flex items-center justify-between mb-6">
            ...
            <button wire:click="closeForm" class="text-gray-400 hover:text-gray-600 bg-gray-50 rounded-full p-2">
        ...
        <form wire:submit="saveAtribut">
            ...
            <div class="mb-6">
                ...
                <select wire:model="selectedKepakaran" class="w-full px-4 py-3 border border-gray-300 rounded-xl focus:ring-2 focus:ring-green-500 focus:border-green-500 text-sm">
                    <option value="">-- Belum Ditentukan --</option>
                    @foreach($listKepakaran as $k)
                    <option value="{{ $k->id }}">{{ $k->nama_kepakaran }} (JF {{ $k->hierarki_level }})</option>
                    @endforeach
                </select>
            </div>
            ...
            <div class="mb-6">
                <label class="block text-sm font-semibold text-gray-700 mb-2">
                    <svg class="w-4 h-4 inline mr-1 text-green-600" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5H7a2 2 0 00-2 2v12a2 2 0 002 2h10a2 2 0 002-2V7a2 2 0 00-2-2h-2M9 5a2 2 0 002 2h2a2 2 0 002-2M9 5a2 2 0 012-2h2a2 2 0 012 2"/></svg>
                    Bidang Keahlian
                ...
                <div class="grid grid-cols-2 gap-2 max-h-52 overflow-y-auto p-1">
                    ...
                    <label class="flex items-center space-x-2 p-2.5 border rounded-xl cursor-pointer transition {{ in_array($bk->id, $selectedBidangKeahlian) ? 'border-green-500 bg-green-50' : 'border-gray-200 hover:border-gray-300' }}"
                           wire:click="toggleBidang({{ $bk->id }})">
                        <div class="w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 {{ in_array($bk->id, $selectedBidangKeahlian) ? 'bg-green-500 border-green-500' : 'border-gray-300' }}">
                            @if(in_array($bk->id, $selectedBidangKeahlian))
                            <svg class="w-3 h-3 text-white" fill="currentColor" viewBox="0 0 20 20"><path fill-rule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clip-rule="evenodd"/></svg>
                            @endif
                        </div>
                        <div class="text-sm">
                            <p class="{{ in_array($bk->id, $selectedBidangKeahlian) ? 'font-medium text-green-800' : 'text-gray-700' }}">{{ $bk->nama_bidang }}</p>
                ...
                <p class="text-xs text-gray-400 mt-1">
                    <a href="{{ route('kajur.data-master.bidang-keahlian') }}" target="_blank" class="text-green-600 hover:underline">+ Kelola Bidang Keahlian</a>
                </p>
            </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        ...
        <div class="bg-white rounded-2xl p-4 shadow-sm border border-green-100">
            ...
            <p class="text-2xl font-bold text-green-700">{{ $dosenDenganKepakaran }}</p>
        </div>
        <div class="bg-white rounded-2xl p-4 shadow-sm border border-green-100">
            <p class="text-xs text-gray-500">Punya Bidang</p>
            <p class="text-2xl font-bold text-green-700">{{ $dosenDenganBidang }}</p>
        </div>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
        <div class="flex items-center justify-between mb-3">
            ...
            <button wire:click="$toggle('quickMode')" class="text-sm {{ $quickMode ? 'text-amber-700' : 'text-green-600' }} hover:underline font-medium">
        ...
        <div class="border-t pt-4">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4">
                ...
                <div>
                    ...
                    <div class="max-h-32 overflow-y-auto border border-gray-300 rounded-lg p-2">
                        ...
                        <label class="flex items-center space-x-2 py-1 text-sm">
                            <input type="checkbox" wire:model="quickBidangKeahlian" value="{{ $bk->id }}" class="w-3.5 h-3.5 text-green-600 rounded">
                ...
                <div class="flex items-end">
                    <button wire:click="applyQuickAssign" class="w-full px-4 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 text-sm font-medium {{ count($selectedDosenIds) == 0 ? 'opacity-50 cursor-not-allowed' : '' }}" {{ count($selectedDosenIds) == 0 ? 'disabled' : '' }}>
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-4 mb-6">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <select wire:model.live="kepakaranFilter" class="px-4 py-2.5 pr-10 border border-gray-300 rounded-xl appearance-none cursor-pointer bg-white">
            ...
            <select wire:model.live="bidangFilter" class="px-4 py-2.5 pr-10 border border-gray-300 rounded-xl appearance-none cursor-pointer bg-white">
    ...
    <div class="bg-green-50 border border-green-200 rounded-xl p-3 mb-4 flex items-center justify-between">
        <div class="flex items-center gap-3">
            <input type="checkbox" wire:model.live="selectAll" class="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500">
            <span class="text-sm text-green-800">Pilih Semua ({{ $totalDosen }})</span>
            @if(count($selectedDosenIds) > 0)
            <span class="text-sm font-medium text-green-700">{{ count($selectedDosenIds) }} terpilih</span>
            @endif
        </div>
    </div>
    @endif
        ...
        <div class="bg-white rounded-2xl shadow-sm border border-gray-100 p-5 hover:shadow-md transition">
            ...
            <div class="flex items-start justify-between mb-4">
                <div class="flex items-center space-x-3">
                    @if($quickMode)
                    <input type="checkbox" wire:model.live="selectedDosenIds" value="{{ $dosen->id }}" class="w-4 h-4 text-green-600 border-gray-300 rounded focus:ring-green-500 flex-shrink-0">
                    @endif
                    <img class="w-12 h-12 rounded-full flex-shrink-0" src="https://ui-avatars.com/api/?name={{ urlencode($dosen->name) }}&background=16a34a&color=fff" alt="">
                    <div class="min-w-0">
                        <p class="font-semibold text-gray-900 text-sm truncate">{{ $dosen->name }}</p>
                        <p class="text-xs text-gray-500">{{ $dosen->nip ?? 'No NIP' }}</p>
                        @if($dosen->prodi)
                        <span class="px-2 py-0.5 bg-green-100 text-green-800 rounded-full text-xs mt-1 inline-block">{{ $dosen->prodi->nama_prodi }}</span>
                        @endif
                    </div>
                </div>
            </div>
            ...
            <div class="mb-3">
                ...
                <span class="px-2 py-1 bg-green-100 text-green-800 rounded-full text-xs font-medium">
                    ...
                    <span class="text-green-500 ml-1">JF {{ $dosen->kepakaran->hierarki_level }}</span>
                </span>
                @else
            ...
            <div class="flex items-center gap-2 pt-3 border-t">
                <button wire:click="openEdit({{ $dosen->id }})" class="flex-1 px-3 py-2 bg-green-700 text-white rounded-lg hover:bg-green-800 text-xs font-medium text-center transition">
